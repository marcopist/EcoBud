...
export default function TransactionSingleScreen({ route, navigation }) {
  ...
  // This fetches the transaction object from the server
  useEffect(() => {
    getTransaction(id)
      .then((transaction) => {
  ...
  // This takes the transaction object and updates the transaction on the server
  useEffect(() => {
    if (transaction) {
      changeTransaction(id, transaction)
        .catch((error) => {
          console.error("Failed to fetch transaction:", error);
        });
    }
